...
export default function RegisterTeam() {
  ...
  // 🔥 SURGICAL SUBMIT - FIXED ALL FIELD NAMES
  const submit = async () => {
    setLoading(true);
    setError('');

    if (!SPACE_ID || !MANAGEMENT_TOKEN) {
        setError("Configuration Error: Hakikisha SPACE_ID na ACCESS_TOKEN zipo kwenye .env");
        setLoading(false);
        return;
    }
    ...
    try {
      ...
      // ✅ FIXED: Player JSON structure matches schema exactly
      const playersPayload = players
        ...
        .map(p => ({
          ...
          photo: p.photoUrl || ""  // ✅ FIXED: "photo" not "photoAssetId"
        }));

      // ✅ FIXED: All field IDs match Contentful schema exactly
      const contentfulData = {
        fields: {
          teamName: { 'en-US': team.name },
          coachName: { 'en-US': team.coach },
          phoneNumber: { 'en-US': team.phone },
          Location: { 'en-US': loc },  // ✅ FIXED: Capital L!
      ...
      // Only add jerseyColor if user filled it
      ...
      if (!response.ok) {
        ...
        let detailedMessage = errorData.message || 'Unknown error';
        if (errorData.details && errorData.details.errors) {
            const fieldError = errorData.details.errors[0];
            detailedMessage = `Field error: ${fieldError.name || 'Unknown'} - ${fieldError.details || 'Invalid data'}`;
        }
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn { background: #a3e635; color: black; font-weight: 700; padding: 12px 24px; border-radius: 8px; border: none; cursor: pointer; font-size: 14px; text-transform: uppercase; transition: 0.2s; display: inline-flex; align-items: center; justify-content: center; gap: 8px; }
        ...
        @media (max-width: 768px) { .grid { grid-template-columns: 1fr !important; } .btn { width: 100%; } }
      `}</style>
      ...
      <div style={{ minHeight: '100vh', background: 'radial-gradient(circle at top, rgba(163,230,53,0.05), #020617 50%)', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {/* MAIN CARD */}
        <div style={{ width: '100%', maxWidth: '800px', background: 'rgba(30,41,59,0.7)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px', padding: '32px', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: '900', color: 'white', marginBottom: '4px', letterSpacing: '1px' }}>USAJILI WA TIMU</h1>
          ...
          {/* ERROR MODAL (GLASS) */}
          {error && (
            <div style={{ background: 'rgba(239,68,68,0.1)', backdropFilter: 'blur(10px)', border: '1px solid rgba(239,68,68,0.3)', color: '#fca5a5', padding: '16px', borderRadius: '12px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px', fontWeight: '600' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: '12px', marginTop: '32px' }}>
                      ...
                      <div>
                        ...
                        <div className="photo" onClick={() => fileRefs.current[p.id]?.click()}>
                           {p.uploading ? <Loader className="spin" size={24} color="#a3e635" /> : p.preview ? <img src={p.preview} alt="Player" /> : <Camera size={20} color="#64748b" />}
                        </div>
                      </div>
                ...
                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '16px', padding: '24px', textAlign: 'left', border: '1px solid rgba(255,255,255,0.05)' }}>
                   <div style={{ background: 'rgba(163,230,53,0.05)', border: '1px solid rgba(163,230,53,0.2)', borderRadius: '12px', padding: '16px', marginBottom: '20px' }}>
                     <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                       ...
                       <button onClick={copy} style={{ background: 'rgba(163,230,53,0.1)', border: '1px solid rgba(163,230,53,0.3)', color: '#a3e635', padding: '12px', borderRadius: '10px', cursor: 'pointer', transition: '0.3s' }} onMouseEnter={(e) => e.currentTarget.style.background = '#a3e635'} onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(163,230,53,0.1)'}>
                   ...
                   <div style={{ fontSize: '13px', color: '#cbd5e1', lineHeight: '1.8' }}>
                     <p style={{ fontWeight: '800', color: 'white', marginBottom: '12px', textTransform: 'uppercase', fontSize: '12px', letterSpacing: '1px' }}>Hatua za Kulipa:</p>
          ...
          {step === 5 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', padding: '30px 0' }}>
              <div style={{ background: 'rgba(163,230,53,0.1)', padding: '20px', borderRadius: '50%' }}>
                <CheckCircle size={56} color="#a3e635" />
              </div>
              <h3 style={{ fontSize: '24px', fontWeight: '900' }}>Tayari Kusubmit?</h3>
        ...
        {success && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(2,6,23,0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
            <div style={{ background: 'rgba(30,41,59,0.7)', border: '1px solid rgba(163,230,53,0.3)', maxWidth: '400px', width: '100%', padding: '40px 32px', borderRadius: '24px', textAlign: 'center', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
              ...
              <h2 style={{ fontSize: '28px', fontWeight: '900', color: 'white', marginBottom: '12px' }}>HONGERA! 🎉</h2>
              <p style={{ color: '#cbd5e1', fontSize: '15px', lineHeight: '1.6', marginBottom: '32px' }}>Usajili wa <strong style={{ color: '#a3e635' }}>{team.name}</strong> umefanikiwa!</p>
