...
// --- FALLBACK DATA ---
const FALLBACK_DATA = {
  hero: [{ location: 'kiomoni', title: "HII GAME NI YETU.", subtitle: "Soka la mtaani lenye hadhi ya kitaifa.", bgImage: "https://images.unsplash.com/photo-1518605336396-6a727c5c0d66?auto=format&fit=crop&q=80&w=1600" }],
...
export const HomePage = () => {
  ...
  // --- STANDARD FETCH (NO RISKY OPTIMIZATION) ---
  useEffect(() => {
    const fetchContentfulData = async () => {
      ...
      try {
        ...
        // SAFE ASSET URL GETTER (No extra params that break links)
        ...
        const hero = heroRes.items.map(i => ({
            ...
            bgImage: getAssetUrl(i.fields.backgroundImage?.sys?.id || i.fields.image?.sys?.id, heroRes.includes)
        }));
        ...
        const news = newsRes.items.map(i => ({
            ...
            image: getAssetUrl(i.fields.image?.sys?.id, newsRes.includes) || "/images/IMG_5866.jpeg",
            location: String(i.fields.location).toLowerCase(), season: i.fields.season
        }));

        const standings = standingsRes.items.map(i => ({
            team: i.fields.teamName, p: i.fields.played, gd: i.fields.goalDifference, pts: i.fields.points, cleanSheets: i.fields.cleanSheets || 0,
        ...
        const sponsors = sponsorRes.items.map(i => ({
            name: i.fields.name, 
            logo: getAssetUrl(i.fields.logo?.sys?.id, sponsorRes.includes), 
  ...
  const s = {
    ...
    // HERO FIXED: Increased height and padding to fit Socials
    hero: { position: 'relative', minHeight: isMobile ? '90vh' : '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid rgba(163,230,53,0.1)', paddingBottom: '40px' },
  ...
  return (
    <div style={s.bg}>
      ...
      {/* HERO SECTION - FIXED SOCIALS VISIBILITY */}
      <div style={s.hero}>
        ...
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '0 20px', width: '100%', maxWidth: '800px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {isGoba2025 ? (
             ...
             <div className="animate-fade-in">
                ...
                <p style={{ fontSize: isMobile ? '16px' : '18px', color: '#cbd5e1', marginBottom: '32px', lineHeight: 1.6 }}>{currentHero.subtitle}</p>
                ...
                <div style={{ 
                    ...
                    flexWrap: 'wrap',
                    padding: '10px',
                    zIndex: 20 // Ensure they are clickable
                }}>
                  {[
                    ...
                    <a key={i} href={S.link} target="_blank" rel="noreferrer" style={{ color: 'white', background: 'rgba(255,255,255,0.15)', width: '50px', height: '50px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', border: '1px solid rgba(255,255,255,0.1)' }}>
                      <S.icon size={24} />
                    </a>
                  ))}
                </div>
         ...
         <div className="marquee-track">
            {[...cmsData.sponsors, ...cmsData.sponsors].map((sp, i) => (
               <div key={i} className="sponsor-marquee-item">
                  <img src={sp.logo} alt={sp.name} style={{ height: '50px', objectFit: 'contain', filter: 'grayscale(100%)', opacity: 0.6 }} />
               </div>
            ))}
         </div>
      </div>
