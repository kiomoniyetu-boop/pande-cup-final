...
const App = () => {
  ...
  const handleFinalSubmit = () => { alert(`Asante ${teamData.coachName}! Ombi la timu ya ${teamData.name} limepokelewa. Tutawasiliana.`); setModalStep(3); };
  ...
  return (
    <>
      ...
      <>
        ...
        {/* 6b. GALLERY */}
        <section id="gallery" style={{ padding: '80px 24px', maxWidth: '1200px', margin: '0 auto', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={styles.sectionHeader}>
              <ImageIcon style={styles.limeText} size={24} />
              <h2 style={styles.sectionTitle}>Matukio <span style={styles.limeText}>{activeSeason}</span></h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '16px' }}>
              {currentGallery.map((img, idx) => (
                  <img key={idx} src={img} className="hover-card" style={{ width: '100%', height: '200px', objectFit: 'cover', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)' }} alt={`Gallery ${idx}`} onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1517466787929-bc90951d0974?auto=format&fit=crop&q=80&w=600"; }} />
              ))}
          </div>
        </section>
      </>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '60px' }}>
          <div>
            ...
            <div style={{ marginBottom: '20px' }}>
                <p style={{ color: '#94a3b8', lineHeight: '1.6', fontSize: '14px', margin: '0 0 16px' }}><strong>Pande Cup</strong> ni chombo cha mabadiliko ya kijamii kinachotumia mpira kama lugha ya umoja na maendeleo.</p>
                <div style={{ borderLeft: '3px solid #a3e635', paddingLeft: '12px', fontStyle: 'italic', color: '#a3e635', fontSize: '13px' }}>"Pamoja sisi ni Pande. Pamoja sisi ni Kiomoni."</div>
            </div>
            <div style={{ display: 'flex', gap: '16px' }}><a href="#" style={{ color: 'white', opacity: 0.7 }}><Instagram size={20} /></a><a href="#" style={{ color: 'white', opacity: 0.7 }}><Facebook size={20} /></a><a href="#" style={{ color: 'white', opacity: 0.7 }}><TikTokIcon size={20} /></a></div>
          </div>
          <div>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, backgroundColor: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', backdropFilter: 'blur(5px)' }}>
          <div style={{ backgroundColor: '#0f172a', border: '1px solid rgba(255,255,255,0.1)', width: '100%', maxWidth: '450px', borderRadius: '24px', padding: '32px', position: 'relative' }}>
            ...
            {modalStep === 1 && (
              <div>
                <h2 style={{ fontSize: '20px', fontWeight: '900', textTransform: 'uppercase', marginBottom: '24px' }}>Sajili <span style={styles.limeText}>Timu</span></h2>
                ...
                <input type="text" placeholder="Jina la Kocha" value={teamData.coachName} onChange={(e) => setTeamData({...teamData, coachName: e.target.value})} style={{ width: '100%', padding: '14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', backgroundColor: '#020617', color: 'white', marginBottom: '12px' }} />
            ...
            {modalStep === 2 && (
                 <div style={{ textAlign: 'center' }}>
                    <h2 style={{ fontSize: '20px', fontWeight: '900', textTransform: 'uppercase', marginBottom: '8px' }}>Kamilisha Malipo</h2>
                    <p style={{ color: '#94a3b8', fontSize: '13px', marginBottom: '24px' }}>Tafadhali lipia ada ya usajili ili kuthibitisha nafasi ya timu yako.</p>
                    ...
                    <button onClick={handleFinalSubmit} style={{ ...styles.buttonPrimary, width: '100%', justifyContent: 'center', backgroundColor: 'white', color: 'black' }}>NIMELIPIA</button>
                 </div>
            )}
            {modalStep === 3 && (
                 <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ color: '#94a3b8', fontSize: '14px' }}>Tutawasiliana nawe muda si mrefu.</p>
